...
macro_rules! text_format {
    ($type:ident based on $format:ident) => {
        impl<'a, T> Into<$crate::djed_format::Text> for $type<&'a T>
        where
            T: ::serde::Serialize,
        {
            fn into(self) -> $crate::djed_format::Text {
                $format::to_string(&self.0).map_err(::anyhow::Error::from)
            }
        }

        impl<T> From<$crate::djed_format::Text> for $type<Result<T, ::anyhow::Error>>
        where
            T: for<'de> ::serde::Deserialize<'de>,
        {
            fn from(value: $crate::djed_format::Text) -> Self {
                match value {
...
macro_rules! binary_format {
    ...
    ($type:ident, $into:path, $from:path) => {
        impl<'a, T> Into<$crate::djed_format::Binary> for $type<&'a T>
        where
            T: ::serde::Serialize,
        {
            fn into(self) -> $crate::djed_format::Binary {
                $into(&self.0).map_err(::anyhow::Error::from)
            }
        }

        impl<T> From<$crate::djed_format::Binary> for $type<Result<T, ::anyhow::Error>>
        where
            T: for<'de> ::serde::Deserialize<'de>,
        {
            fn from(value: $crate::djed_format::Binary) -> Self {
                match value {
...
macro_rules! text_format_is_an_error {
    ($type:ident) => {
        use $crate::{djed_format::FormatError, text_format};
